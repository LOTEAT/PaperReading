<!--
 * @Author: LOTEAT
 * @Date: 2024-08-19 20:00:31
-->
## SamplingResult 
- 前置知识：PyTorch
- [代码链接](https://github.com/open-mmlab/mmdetection)
### 1. Code
在`mmdetection`中`SamplingResult`类的主要作用包装sample后的结果。在这个类中，主要函数是`random`和，`random`函数是用于debug或者测试用的。`SamplingResult`在构造函数会将正负样本从proposal中抽取出来。
