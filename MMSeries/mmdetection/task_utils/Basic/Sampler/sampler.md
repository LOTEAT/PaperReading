<!--
 * @Author: LOTEAT
 * @Date: 2024-08-19 17:05:26
-->
## Sampler 
- 前置知识：PyTorch
- [代码链接](https://github.com/open-mmlab/mmdetection)
### 1. Code
在`mmdetection`中`BaseSampler`类的主要作用是对正负样本进行采样。
使用不同的策略，为bbox赋予正标签（类别标签）或者是负标签（背景标签）。可以看到，基类的代码十分简单，提供了一个sample函数供后续子类调用。要注意的是，在sample函数中，默认是将gt框也加入被采样的集合之中。
```python
