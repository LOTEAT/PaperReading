...
### 1. Max IoU Assigner
这个类的核心功能在于将proposal region和gt框的IoU大于阈值的标注为正标签，小于阈值的标注为负标签，也就是背景标签。

### 2. Code
`MaxIoUAssigner`涉及到一些辅助函数，我们首先看`_perm_box`函数。这个函数的主要作用就是对边界框进行扰动，并确保扰动后的边界框与原始边界框之间的IoU（交并比）超过一定的阈值。讲解在代码注释中。
```python
def _perm_box(bboxes,
    ...
    is_valid = True
    N = bboxes.size(0)
    # perm_factor 是一个随机生成的缩放因子，范围在[1 - perm_range, 1 + perm_range]之间
    perm_factor = bboxes.new_empty(N, 4).uniform_(1 - perm_range,
                                                  1 + perm_range)
    bboxes *= perm_factor
    new_wh = bboxes[:, 2:] - bboxes[:, :2]
    # 如果任何一个边界框的宽度或高度小于或等于0，则设置is_valid为False，表示这个扰动无效
    if (new_wh <= 0).any():
        is_valid = False
    iou = iou_calculator(ori_bboxes.unique(dim=0), bboxes)
    if (iou < iou_thr).any():
        is_valid = False
    # 如果当前扰动无效并且递归计数器未达到最大次数，则递归调用_perm_box函数
    ...
    return bboxes
```
`perm_repeat_bboxes`的目的是对输入的边界框进行扰动，特别是对重复的边界框进行扰动处理
```python
def perm_repeat_bboxes(bboxes, iou_calculator=None, perm_repeat_cfg=None):
    ...
    return bboxes
```
结合上述两个函数，就可以理解`MaxIouAssigner`了，讲解在代码注释中。
```python
class MaxIoUAssigner(BaseAssigner):
    ...
    def __init__(self,
        ...
        # 这些参数变量官方注释写得并不清晰，可以结合assign函数理解
    ...
    def assign_wrt_overlaps(self, overlaps: Tensor,
        ...
        # self.neg_iou_thr为负样本的阈值，可以输入float表示在[0, self.neg_iou_thr)区间内为负样本
        # 也可以输入元组，表示在[lb, ub)区间内味负样本
        # 负样本标签设置为0
        ...
        # 设置正样本，加一是因为索引从0开始，但是要避免和负样本的0重复
        pos_inds = max_overlaps >= self.pos_iou_thr
        assigned_gt_inds[pos_inds] = argmax_overlaps[pos_inds] + 1
        # 先前是根据anchor与gt的IoU判断anchor属于哪一个类别
        # 但是可能会有些类别的gt与anchor重合的区域较小
        # 因此，采用match_low_quality策略，对于每一个gt框都选择一个与其overlap最大的anchor并赋予标签
        # 不过，这也可能会造成标签覆盖
